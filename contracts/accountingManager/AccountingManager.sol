...
import "./WithdrawErrorsHandler.sol";
import {Math} from "@openzeppelin/contracts-5.0/utils/math/Math.sol";
...
**/
contract AccountingManager is IAccountingManager, ERC20, ReentrancyGuard, NoyaGovernanceBase {
    ...
    uint256 private constant WITHDRAWAL_MAX_FEE = 5e4;
    uint256 private constant MANAGEMENT_MAX_FEE = 1e5;
    uint256 private constant PERFORMANCE_MAX_FEE = 5e5;
    ...
    WithdrawErrorHandler public withdrawErrorsHandler;
    ...
    NoyaGovernanceBase(PositionRegistry(p._registry), p._vaultId)
    {
        ...
        minWithdrawalAmount = p._minWithdrawAmount;
        minDepositAmount = p._minDepositAmount;
    }

    function setMinAmounts(uint256 _minWithdrawalAmount, uint256 _minDepositAmount) public onlyMaintainer {
        minWithdrawalAmount = _minWithdrawalAmount;
        minDepositAmount = _minDepositAmount;
        emit MinAmountsChanged(_minWithdrawalAmount, _minDepositAmount);
    }
    ...
    function updateWithdrawErrorsHandler(address _withdrawErrorsHandler) public onlyMaintainer {
        withdrawErrorsHandler = WithdrawErrorHandler(_withdrawErrorsHandler);
        emit WithdrawErrorsHandlerUpdated(_withdrawErrorsHandler);
    }
    ...
    **/
    function deposit(address receiver, uint256 amount, address referrer) public nonReentrant whenNotPaused {
        ...
        if (receiver == address(0)) {
            revert NoyaAccounting_INVALID_ADDRESS();
        }
        ...
        if (totalAssets() > depositLimitTotalAmount) {
    ...
    */
    ...
    function executeWithdraw(uint256 maxIterations) public onlyManager nonReentrant whenNotPaused {
        ...
        while (
            ...
            if (baseTokenAmount > 0) {
                try this.attemptTransfer(baseToken, data.receiver, baseTokenAmount) {}
                catch Error(string memory reason) {
                    success = false;
                    baseToken.safeTransfer(address(withdrawErrorsHandler), baseTokenAmount);
                    withdrawErrorsHandler.withdrawalError(address(baseToken), data.receiver, baseTokenAmount, reason);
                }
            }
            emit ExecuteWithdraw(
                firstTemp, data.owner, data.receiver, shares, data.amount, baseTokenAmount, block.timestamp, success
            );
    ...
    function resetMiddle(uint256 newMiddle, bool depositOrWithdraw) public onlyManagerOrWatcher {
        if (depositOrWithdraw) {
            ...
            if (newMiddle != withdrawQueue.first || currentWithdrawGroup.isStarted) {
                revert NoyaAccounting_INVALID_AMOUNT();
            }
            currentWithdrawGroup.totalCBAmount = 0;
    ...
    function retrieveTokensForWithdraw(RetrieveData[] calldata retrieveData, address connector, bytes memory addLPdata)
    public
    onlyManager
    nonReentrant
    {
        ...
        if (amountAskedForWithdraw_temp > neededAssets) {
            if (registry.isAnActiveConnector(vaultId, connector)) {
                uint256[] memory amounts = new uint256[](1);
                amounts[0] = amountAskedForWithdraw_temp - neededAssets;
                address[] memory tokens = new address[](1);
                tokens[0] = address(baseToken);
                IConnector(connector).addLiquidity(tokens, amounts, addLPdata);
            } else {
    ...
    function decimals() public view override returns (uint8) {
        return IERC20Metadata(address(baseToken)).decimals();
    }
}
